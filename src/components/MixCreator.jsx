...
const MixCreator = ({ records, onClose, onMixCreated, onPlayTrack }) => {
  ...
  const [selectedSeedTrack, setSelectedSeedTrack] = useState(null);
    ...
    // Debug: Log when success state changes
    useEffect(() => {
        if (success) {
            console.log('✅ Success state set, playlist created:', success);
        }
    }, [success]);
  ...
  const [matchingSettings, setMatchingSettings] = useState({
    ...
    // Check if server has refresh token configured
    const checkServerToken = async () => {
      try {
        ...
        // Server might not be running or token not configured
        setHasServerToken(false);
      }
    };

    // Check immediately and also retry after a short delay (in case server is starting)
    checkServerToken();
    const retryTimer = setTimeout(checkServerToken, 2000);
  ...
  const handleGenerateMix = async () => {
    ...
    if (mixChain.length === 0) {
      ...
      const description = `DJ mix from my vinyl collection (${mixChain.length} tracks) - Mixed by Camelot key, BPM, danceability, and acousticness`;
            ...
            // Create new playlist
      // accessToken can be null if server has refresh token configured
            const createResponse = await fetch('/api/spotify-create-playlist', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
          accessToken: accessToken || undefined, // Only send if available, server will use refresh token otherwise
                    playlistName,
                    description,
                    trackUris
                })
            ...
            const successData = {
                ...
                uri: playlistData.playlist.uri,
                trackCount: trackUris.length,
                tracks: mixChain // Include full track data for preview
            };
        ...
        >
          ...
          {/* Step 1: Select Starting Track */}
                ...
                <p className="selection-info">
                  Choose a track from your collection to start the mix. The system will find tracks that mix well based on Camelot key, BPM, danceability, and acousticness.
                </p>
                    ...
                    <div className="tracks-table">
                      ...
                      <div className="tracks-table-body">
                        {mixChain.map((track, index) => {
                          const camelot = getCamelotKey(track.getsongbpm?.key);
                          const prevTrack = index > 0 ? mixChain[index - 1] : null;
                          const mixabilityInfo = prevTrack ? findMixableTracks(prevTrack, [track], { minScore: 0, maxResults: 1, ...matchingSettings })[0] : null;
                          const durationMs = parseDurationToMs(track.duration || track.duration_ms);
              ...
              {!accessToken && !hasServerToken ? (
                                    <div className="mix-creator-section">
                                        <div className="mix-creator-auth">
                                            <p>Connect to Spotify to save this playlist</p>
                                            <button
                  ...
                  disabled={isCreating || (!accessToken && !hasServerToken) || mixChain.length === 0}
                                    >
